...
// Sample Teltonika AVL packet (Codec 8) - corrected preamble
...
console.log('🔍 Debug AVL Packet Test');
console.log('========================');
...
// Check preamble
const preamble = sampleAVLPacket.readUInt32BE(offset);
console.log(`📍 Preamble: 0x${preamble.toString(16).padStart(8, '0')} (expected: 0x00000000)`);
offset += 4;

// Check data field length
const dataFieldLength = sampleAVLPacket.readUInt32BE(offset);
console.log(`📍 Data field length: ${dataFieldLength} bytes`);
offset += 4;

// Check IMEI length
const imeiLength = sampleAVLPacket.readUInt8(offset);
console.log(`📍 IMEI length: ${imeiLength} characters`);
offset += 1;

// Check IMEI
const imei = sampleAVLPacket.toString('ascii', offset, offset + imeiLength);
console.log(`📍 IMEI: ${imei}`);
offset += imeiLength;

// Check codec ID
const codecId = sampleAVLPacket.readUInt8(offset);
console.log(`📍 Codec ID: 0x${codecId.toString(16)} (expected: 0x08)`);
offset += 1;

// Check number of records
const numberOfRecords = sampleAVLPacket.readUInt8(offset);
console.log(`📍 Number of records: ${numberOfRecords}`);
offset += 1;

console.log(`📍 Remaining bytes: ${sampleAVLPacket.length - offset}`);
