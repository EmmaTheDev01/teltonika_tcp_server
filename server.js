...
const TELTONIKA_PREAMBLE = 0x000000FF;
  ...
  try {
    ...
    if (preamble !== TELTONIKA_PREAMBLE) {
      log('error', 'Invalid Teltonika preamble', { 
        ...
        expected: TELTONIKA_PREAMBLE.toString(16) 
      });
      
      // Try to find the preamble in the buffer
      const preambleIndex = buffer.indexOf(Buffer.from([0x00, 0x00, 0x00, 0xFF]));
      if (preambleIndex > 0) {
        log('info', `Found preamble at offset ${preambleIndex}, adjusting...`);
    ...
    for (let i = 0; i < numberOfRecords; i++) {
      ...
      if (record) {
        ...
        log('error', `Failed to parse AVL record ${i}`);
        return null;
      }
    }
    ...
    log('error', 'Error parsing Teltonika AVL packet', error);
    return null;
  }
}
